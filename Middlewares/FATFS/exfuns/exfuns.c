...
u8 exfuns_init(void)
{
    ...
    if(i == FF_VOLUMES&&file&&ftemp&&fatbuf)       //申请有一个失败,即失败
        return 0;
    else
        return 1;
}
 ...
 * 
 * @param fname 文件名
 * @return u8 0XFF:表示无法识别的文件类型编号 其他:高四位表示所属大类,第四位表示所属小类
 */
...
/**
 * @brief 文件复制,将psrc文件,copy到pdst,注意文件大小不要超过4GB
 * 
...
u8 exf_copy(u8(*fcpymsg)(u8*pname, u8 pct, u8 mode), u8 *psrc, u8 *pdst, u32 totsize, u32 cpdsize, u8 fwmode)
{
    ...
    if(fsrc==NULL || fdst==NULL || fbuf==NULL)  //前面的值留给fatfs
        res = 100;
    else
    {
        if(fwmode==0)  //不覆盖
 ...
 * 
 * @param dpfn 路径
 * @return u8* 0:路径是一个卷标号 其他:文件夹名字首地址
 */
u8* exf_get_src_dname(u8* dpfn)
{
    ...
    if(temp < 4)
        return 0;
    while((*dpfn!=0x5c) && (*dpfn!=0x2f))  //追述到倒数第一个"\"或者"/"处
        dpfn--;
    return ++dpfn;
}
 ...
 * 
 * @param fdname 文件夹地址
 * @return u32 0:文件夹大小为0,或者读取过程中发生了错误 其他:文件夹大小
 */
        ...
        if(res == 0)
        {
            ...
            if(res == 0)  //打开目录成功 
            {
                while(res == 0)  //开始复制文件夹里面的东东
                {
                    ...
                    if(res!=FR_OK || finfo->fname[0]==0)  //错误了/到末尾了,退出
                        break;
                    if(finfo->fname[0] == '.')            //忽略上级目录
                        continue;
...
/**
 * @brief 文件夹复制,将psrc文件夹,copy到pdst文件夹,注意文件夹大小不要超过4GB,必须形如"X:"/"X:XX"/"X:XX/XX"之类的.而且要实现确认上一级文件夹存在
 * 
 * @param fcpymsg 函数指针,用于实现拷贝时的信息显示
 *                pname:文件/文件夹名
 *                pct:  百分比
 ...
 * @param fwmode  文件写入模式 0:不覆盖原有的文件 1:覆盖原有的文件
 * @return u8 0:成功 其他:错误代码(0xFF:强制退出)
 */
