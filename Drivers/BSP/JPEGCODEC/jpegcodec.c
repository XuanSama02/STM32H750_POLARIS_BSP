...
//JPEG规范(ISO/IEC 10918-1标准)的样本量化表,用于获取JPEG质量
...
/**
 * @brief JPEG硬件解码输入DMA配置函数
 * 
 * @param meminaddr JPEG输入DMA存储器地址
...
{ 
    ...
    MDMA_Channel7->CTBR  = 17<<0;             //MDMA的硬件触发通道17触发inmdma,通道17=JPEG input FIFO threshold,详见<STM32H7xx参考手册>550页,table 91
    addrmask = meminaddr&0XFF000000;          //获取掩码
    if(addrmask==0X20000000 || addrmask==0)   //使用AHBS总线访问DTCM/ITCM
...
/**
 * @brief JPEG硬件解码输出DMA配置
 * 
 * @param memoutaddr JPEG输出DMA存储器地址
...
{ 
    ...
    MDMA_Channel6->CTBR = 19<<0;              //MDMA的硬件触发通道17触发inmdma,通道17=JPEG input FIFO threshold,详见<STM32H7xx参考手册>550页,table 91
...
/**
 * @brief MDMA中断服务函数,处理硬件JPEG解码时输入/输出数据流
 * 
 */
...
u8 jpeg_core_init(jpeg_codec_typedef *tjpeg)
{
    ...
    {
        tjpeg->inbuf[i].buf = mymalloc(SRAMDTCM, JPEG_DMA_INBUF_LEN);  //分配内存
        if(tjpeg->inbuf[i].buf == NULL)  //内存分配失败,退出
        {
...
void jpeg_core_destroy(jpeg_codec_typedef *tjpeg)
{
    ...
    for(i=0; i<JPEG_DMA_INBUF_NB; i++)   //释放内存
...
/**
 * @brief 启动JPEG OUT DMA,开始输出YUV数据
 * 
 */
...
/**
 * @brief 得到jpeg图像质量,在解码完成后,可以调用并获得正确的结果
 * 
 * @return u8 图像质量:0~100
 */
...
/**
 * @brief 利用DMA2D,将JPEG解码的YUV数据转换成RGB数据,硬件解码,速度非常快
 * 
 * @param tjpeg jpeg解码结构体
 * @param pdst  输出数组首地址
 * @return u8 0:完成 1:超时退出
 */
